...
class PRBCD:
    ...
    def __init__(self, data, model=None,
        ...
        if model is None:
            model, clean_test, logits = self.pretrain_model()
    ...
    def pretrain_model(self, model=None):
        ...
        output, logits = model.predict()
        labels = labels.to(device)
        clean_test = self.get_perf(output, labels, data.test_mask, verbose=0)[1]
        print(f"{model.name} Test set results:", clean_test)
        ...
        return model, clean_test, logits
    ...
    def sample_final_edges(self, n_perturbations):
        ...
        for i in range(self.max_final_samples):
            ...
            with torch.no_grad():
                output, logits = self.model.forward(feat, edge_index, edge_weight)
    ...
    def attack(self, edge_index=None, edge_weight=None, ptb_rate=0.1):
        ...
        with torch.no_grad():
            output, logits = model.forward(feat, self.edge_index)
        ...
        # print('--labels', pred[data.valid_mask])
        ...
        for it in tqdm(range(epochs)):
            ...
            output, logits = model.forward(feat, edge_index, edge_weight)
            ...
            with torch.no_grad():
                ...
                edge_index, edge_weight = self.get_modified_adj()
                output, logits = model.predict(feat, edge_index, edge_weight)
        ...
        output, logits = model.predict(feat, edge_index, edge_weight)
        print('Test:')
        loss_test, acc_test = self.get_perf(output, gt_labels, data.test_mask)
        print('Validatoin:')
        self.get_perf(output, gt_labels, data.val_mask)
        return edge_index, edge_weight, acc_test
