...
class GCN(nn.Module):
    ...
    def forward(self, x, adj):
        ...
        x_logits = self.gc2(x, adj)
        return F.log_softmax(x_logits, dim=1), x_logits
    ...
    def _train_without_val(self, labels, idx_train, train_iters, verbose):
        ...
        for i in range(train_iters):
            optimizer.zero_grad()
            output, logits = self.forward(self.features, self.adj_norm)
    ...
    def predict(self, features=None, adj=None):
        ...
        if features is None and adj is None:
            out, logits = self.forward(self.features, self.adj_norm)
            return out, logits
