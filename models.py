...
from train_utils import train, test, get_optimizer
...
def get_model(args):
    if args.model_name == 'MLP':
        ...
        return GCN(args, args.num_layers, args.input_dim, args.hidden_dimension, args.num_classes, args.dropout, args.norm)
...
class GCN(torch.nn.Module):
    def __init__(self, args, num_layers, input_dim, hidden_dimension, nclass, dropout, norm=None) -> None:
        super().__init__()
        ...
        self.hidden_sizes = [hidden_dimension]
        self.with_relu = True
        self.with_bias = True
        self.output = None
        self.args = args
        if num_layers == 1:
            self.convs.append(GCNConv(input_dim, nclass, cached=False,
                             normalize=True))
        else:
            ...
            self.convs.append(GCNConv(hidden_dimension, nclass, cached=False, normalize=True))

    def forward(self, data):
        if self.args.attack != 'None':
            if self.args.attack == 'PRBCD':
                data.edge_index = data.edge_index
            else:
                data.edge_index = torch.nonzero(data.edge, as_tuple=False).t()

        x, edge_index, edge_weight= data.x, data.edge_index, data.edge_weight
        # print('GCN-adj', edge_index.size())
        for i in range(self.num_layers):
            x = F.dropout(x, self.dropout, training=self.training)
    ...
    def fit(self, args, data, model, device, train_iters=200):
...
def sbert(device):
    model = SentenceTransformer('all-MiniLM-L6-v2', cache_folder='./localscratch/gk/huggingface', device=device).to(device)
    return model 
