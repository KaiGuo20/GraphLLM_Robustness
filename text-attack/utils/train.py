...
from utils.util import logger, args, set_seed, root_dir
import attacks.models as models

class NewDataset(Dataset):
    def __init__(self, path):
        cache_path = path.split(".pkl")[0] + '_tokenized.pkl'
        self.max_len = 512
...
if __name__ == '__main__':
    train_data = NewDataset(args.train_data)
    test_data = NewDataset(args.test_data)
    set_seed(args)

    model = models.BertC(dropout=args.dropout, num_class= 7) # cora: 7; pubmed: 3; Arxiv: 40
