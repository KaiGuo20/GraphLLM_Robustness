...
parser.add_argument('--data_type', type=str, default='fixed', choices=['fixed', 'finetuned'])
parser.add_argument('--model', type=str, default='sbert', choices=['sbert', 'e5', 'llama', 'bow', 'tfidf'], help='embedding model')
    ...
    if args.edge_ptb_rate == 0 and args.node_ptb_rate == 0:
        ...
        if args.save:
            if args.data_type == 'finetuned':
                torch.save(data, f'gcn_results/{args.dataset}_{args.model}/edge_{args.edge_ptb_rate}_node_{args.node_ptb_rate}_{args.sample_choice}/finetuned_split_{split}.pt') #
            else:
                torch.save(data, f'gcn_results/{args.dataset}_{args.model}/edge_{args.edge_ptb_rate}_node_{args.node_ptb_rate}_{args.sample_choice}/split_{split}.pt') #
...
if __name__ == "__main__":
    ...
    if not os.path.exists(f'gcn_results/{args.dataset}_{args.model}'):
        os.mkdir(f'gcn_results/{args.dataset}_{args.model}')
    if not os.path.exists(f'gcn_results/{args.dataset}_{args.model}/edge_{args.edge_ptb_rate}_node_{args.node_ptb_rate}_{args.sample_choice}'):
        os.mkdir(f'gcn_results/{args.dataset}_{args.model}/edge_{args.edge_ptb_rate}_node_{args.node_ptb_rate}_{args.sample_choice}')
    ...
    if args.save:
        with open(f'gcn_results/{args.dataset}_{args.model}/{args.data_type}_edge_{args.edge_ptb_rate}_node_{args.node_ptb_rate}_{args.sample_choice}.json','w') as f:
            json.dump(result,f) 
