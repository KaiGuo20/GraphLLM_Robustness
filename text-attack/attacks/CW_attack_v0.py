...
from utils.util import args, logger

class CarliniL2:

    def __init__(self, targeted=False, search_steps=None, max_steps=None, cuda=True, debug=False, num_classes=7):
    ...
    def _optimize(self, optimizer, model, input_var, modifier_var, target_var, scale_const_var, input_token=None):
        # print(torch.sum(self.itereated_var**2))
        ...
        if self.mask is None:
            ...
            # input_adv.register_hook(save_grad('input_adv'))
            # i0 = None
            # j0 = None
            ...
            for i in range(input_adv.size(0)):
                ...
                for j in range(add_start, add_end):
                    ...
                    similar_wv = model.model.embeddings.word_embeddings(torch.LongTensor(self.wv[self.seq[i][j].item()]).cuda())[:100] # max num candidates
            ...
            if args.debug_cw:
                ...
                print("out:", adv_seq)
                print("out embedding:", model.model.embeddings.word_embeddings(adv_seq))
                out = model(input_ids = adv_seq, seq_len = self.seq_len)['pred']
                print("out:", out)
