...
class NodePRBCD:
    def __init__(self, data, model=None, text_encoder = None, sample_choice = "random", 
            ...
            confidence = 0, max_candidates = 50,
        ...
        self.modified_nodes: torch.Tensor = None # 1-dim; keep records of perturbed nodes index
    ...
    def pretrain_model(self, model=None):
        ...
        output = model.predict(self.data.x, self.edge_index, self.edge_weight)
    ...
    def optimize_feats(self, labels, edge_index, edge_weight):
        ...
        for i, batch in tqdm(enumerate(loader)):
            ...
            input_embeds = self.word_embeds[batch_seqs.view(-1)].view(batch_size, self.max_len, self.word_embed_size) # batch * 512 * 368
            input_embeds.requires_grad = False
            batch_embeds = torch.tensor(input_embeds)
            batch_candidates_embeds = self.word_embeds[batch_candidates.view(-1)].view(batch_size, self.max_len, self.max_candidates, self.word_embed_size) # batch * 512 * 10 * 368
            ...
            for step in range(4):
                input_adv = batch_embeds + mask * perturbation # batch * 512 * 384
                # find and project
                dist = torch.zeros((batch_size, self.max_len, self.max_candidates)).to(self.device) # self.word_embeds.shape[0]
                for b in range(batch_size):
                    input_b = input_adv[b]
                    for l in range(seq_lens[b]):
                        # ipdb.set_trace()
                        idx = self.iterated_node_tokens.data[index][b][l]

                        dist.data[b][l]= torch.mv(batch_candidates_embeds[b,l], input_b[l]).squeeze()
                        tg = torch.mv(self.word_embeds[idx].unsqueeze(0), input_b[l])
                        dist.data[b][l] = dist.data[b][l] - tg

                        # dist.data[b][l]= torch.mv(self.word_embeds, input_b[l]).squeeze()
                        # dist.data[b][l] = dist.data[b][l] - dist.data[b][l][idx]
                        
                
                # dist = torch.norm((self.word_embeds - input_adv.unsqueeze(2)), 2, -1)
                ...
                # ipdb.set_trace()
                ori_input_adv = input_adv
                batch_iterated = self.iterated_node_tokens.data[index]
                for b in range(batch_size):
                    for l in range(seq_lens[b]):
                        # ipdb.set_trace()
                        idx = dist[b][l].argsort(descending = True)[0]
                        swap = batch_candidates_embeds[b,l][idx]
                        # swap = self.word_embeds[idx]
                        # ipdb.set_trace()
                        if (swap.sum() != 0):
                            input_adv.data[b][l] = swap
                            # ipdb.set_trace()
                            # batch_iterated[b, l] = idx.data
                            batch_iterated[b, l] = batch_candidates[b,l][idx]
                            # print(batch_iterated[b, l] == replace)
                # input_adv.data = batch_embeds.data = self.word_embeds[new_index].view(input_adv.shape)
                # self.iterated_node_tokens[index] = self.word_embeds[new_index].view(new_words_index.shape)
                # ipdb.set_trace()
                self.iterated_node_tokens.data[index] = batch_iterated
                ...
                output = self.model.predict(all_feat, edge_index, edge_weight) # batch.edge_index.detach(), batch.edge_weight.detach()
            ...
            self.feat = all_feat.detach().clone()
    ...
    def attack(self, edge_index=None, edge_weight=None, edge_ptb_rate=0.1):
        ...
        for it in tqdm(range(epochs)):
            ...
            if it % 4 == 0 and self.node_search_space_size:
                ...
                self.optimize_feats(labels, edge_index, edge_weight)
            ...
            if edge_ptb_rate:
                ...
                output = model.forward(self.feat.detach(), edge_index, edge_weight)
        ...
        output = model.predict(self.feat, edge_index, edge_weight)
