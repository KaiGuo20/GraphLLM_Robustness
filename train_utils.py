...
def train(model, data, optimizer, loss_fn, train_mask, val_mask):
    ...
    val_acc = test1(model, data, False, val_mask)
...
@torch.no_grad()
def test1(model, data, return_embeds, mask):
    model.eval()
    # model.model.initialized = False
    out = model(data.x, data.edge)
